...
TARGET_FLOW_RATE = 0.15  # mL/min

# The start flow rate (without proper calibration) in m^3/s
START_FLOW_RATE = 1.6466666666666674e-09

EXPECTED_WEIGHT_CHANGE_RATE = 0.15  # g/min
...
LOWER_LIM = 1.84166668e-10  # m^3/s

# upper flow rate lim in m^3/s
UPPER_LIM = 2.766666666666667e-6  # m^3/s

# Write the data into a file with the following path
os.makedirs("results", exist_ok=True)
FILEPATH = f"results/{datetime.datetime.now()}_Bleed_data.csv"
...
async def prepare_pump(pump: core.Service):
    ...
    await pump.pump_actuator_controller_1.stop()
    await asyncio.sleep(1)
    await pump.pump_actuator_controller_1.set_flow_rate(flow_rate=START_FLOW_RATE)
    await asyncio.sleep(1)

    # Start asynchronously so the loop can continue without blocking on stream startup.
    asyncio.create_task(pump.pump_actuator_controller_1.start(flow_rate=START_FLOW_RATE))

    logger.info("Started pump")


async def set_flow_rate(pump: core.Service, flow_rate: float):
    await pump.pump_actuator_controller_1.set_flow_rate(flow_rate=flow_rate)


async def subscribe_flow_rate(subscription):
    global current_flow_rate
    global error_counter_flow
...
async def subscribe_flow_rate_with_timeout(pump: core.Service):
    ...
    async with await pump.pump_actuator_controller_1.subscribe_flow_rate() as subscription:
        try:
...
async def subscribe_weight(subscription):
    ...
    async for item in subscription:
        if isinstance(item, float) or isinstance(item, int):
            ...
            stable_weight_response = item.get("StableWeightResponse")
            weight_value = stable_weight_response.get("WeightValue") if isinstance(stable_weight_response, dict) else None
            value = weight_value.get("value") if isinstance(weight_value, dict) else None
            if value is None:
...
async def subscribe_weight_with_timeout(balance: core.Service):
    # Re-open the stream if it stalls for too long.
    async with await balance.balance_service.subscribe_current_weight_value() as subscription:
        try:
...
async def run():
    ...
    pump = await connect(name="Reglo ICC 4x ALE 2")
    ...
    if channel_addressing is False:
        ...
        while True:
            ...
            if channel_addressing is True:
                break
            i -= 1
            if i == 0:
                raise RuntimeError("Changing channel addressing setting unsuccessful.")

    balance = await connect(name="MT_Viper_SW")
        ...
        async for iteration in iterate(INTERVAL, DURATION):
            ...
            if len(history) >= N_LAST:
                ...
                if abs(actual_flow_rate) < 1e-12:
                    ...
                    correction_factor = TARGET_FLOW_RATE / actual_flow_rate
...
async def main():
    global pump_subscription, balance_subscription
    try:
