...
TARGET_FLOW_RATE = 0.66  # mL/min

# The start flow rate (without proper calibration) in m^3/s
START_FLOW_RATE = 7.374613387276743e-09

EXPECTED_WEIGHT_CHANGE_RATE = 0.66  # g/min
...
UPPER_LIM = 2.766666666666667e-7  # m^3/s

# Write the data into a file with the following path
os.makedirs("results", exist_ok=True)
FILEPATH = f"results/{datetime.datetime.now()}_Filtrate_data.csv"
...
async def prepare_pump(pump: core.Service):
    ...
    await pump.pump_actuator_controller_3.stop()
    await asyncio.sleep(1)
    await pump.pump_actuator_controller_3.set_flow_rate(flow_rate=START_FLOW_RATE)
    await asyncio.sleep(1)

    # Start asynchronously so the loop can continue without blocking on stream startup.
    asyncio.create_task(pump.pump_actuator_controller_3.start(flow_rate=START_FLOW_RATE))

    logger.info("Started pump")


async def set_flow_rate(pump: core.Service, flow_rate: float):
    await pump.pump_actuator_controller_3.set_flow_rate(flow_rate=flow_rate)
...
async def subscribe_flow_rate_with_timeout(pump: core.Service):
    ...
    async with await pump.pump_actuator_controller_3.subscribe_flow_rate() as subscription:
        try:
...
async def run():
    ...
    pump = await connect(name="Reglo ICC 4x ALE 1")
...
async def main():
    ...
    try:
        ...
        async with client.Client():
            pump = await connect(name="Reglo ICC 4x ALE 1")
            await pump.pump_actuator_controller_3.stop()
            logger.warning("Pump stopped.")
            if pump_subscription is not None:
                pump_subscription.cancel()
    except asyncio.CancelledError:
        logger.warning("Top-level task was cancelled.")
        async with client.Client():
            pump = await connect(name="Reglo ICC 4x ALE 1")
            await pump.pump_actuator_controller_3.stop()
            logger.warning("Pump stopped.")
